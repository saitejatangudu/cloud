...
# From our files
from util_dataload import CustomDataset, get_livefb_train_data, get_livefb_annotation_data
from util_networks import *
from exp2_test_2fc import exp2_2_test_function
...
class DNNIter(nn.Module):
    def __init__(self, config):
        ...
        if self.config.network_type == 'regressor':
            ...
            self.aggregator = BasicMultiheadAttentionAggregator(
                self.config.embed_dim, self.config.num_mha_heads, regressor_bool=False).to(self.device)
        ...
        if self.config.model_description == 'internlm-vl':
            self.model = get_internLM_model(self.config)
        elif self.config.model_description == 'internlm-vl2':
            self.model = get_internLM_v2_model(self.config)
        elif self.config.model_description == 'internlm-quantised':
    ...
    def init_dataloaders(self):
        ...
        processed_train_data = CustomDataset(
            df_data=train_data, img_dir=self.config.img_dir, model='internlm_vl', data_transform='normal_transform')
        processed_test_data = CustomDataset(
            df_data=test_data, img_dir=self.config.img_dir, model='internlm_vl', data_transform='normal_transform')
    ...
    def train_model(self):
        ...
        for iteration in tqdm(range(start_iteration, total_iterations + 1)):
            ...
            if self.config.logit_processing_type == 'init':
                hidden_states = get_qformer_output(self.model, img_input)
...
def configuration_params():
    ...
    parser.add_argument('--default_device', type=int, default=5)
    ...
    parser.add_argument('--results_dir', type=str,
                        default='/scratch/sanjotst/mse_runs_replication/')
    ...
    parser.add_argument('--exp description', type=str,
                        default='MSE with batch size 16, replication of mse baseline internlm-vl with older parameters')
