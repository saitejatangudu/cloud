...
class DNNIter(nn.Module):
    ...
    def init_dataloaders(self):
        ...
        processed_train_data = CustomTrainDatasetAnnotatedLIVEFB(
            df_data=train_data, annotation_matrix_dir=self.config.annotation_directory,  synthetic_img_dir=self.config.synthetic_img_dir)
    ...
    def train_model(self):
        ...
        for iteration in tqdm(range(start_iteration, total_iterations + 1)):
            ...
            #     attention_output_dists, annotator_matrix, 4, 4)
            # Compute each loss separately
            loss1 = loss_fn1(attention_output_dists, 2, 5)
            loss2 = loss_fn2(attention_output_dists, annotator_matrix, 2, 5)
...
def configuration_params():
    ...
    parser.add_argument('--results_dir', type=str,
                        default='/scratch/sanjotst/triplet_loss_runs')
    ...
    parser.add_argument('--experiment description', type=str,
                        default='loss functions are loss_fn1 = LevelBlendedOrderingLoss() loss_fn2 = Opt1_DistBlendedOrderingLoss() ')
