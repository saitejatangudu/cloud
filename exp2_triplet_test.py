# handles test for all triplet loss variants

from util_get_internlm_logits import get_init_logits, get_sentence_logits
from util_dataload import *
from util_networks import get_internLM_model, BasicMultiheadAttentionAggregator, ComplexMultiheadAttentionAggregator, BasicMultiheadAttentionAggregator1, NormalRegressor1, NormalRegressor
...
def init_test_dataloaders(config):
    ...
    processed_test_data = CustomDataset(config.img_dir, test_data)
    pooled_test_loader = DataLoader(
        processed_test_data, batch_size=16, shuffle=False)
    return pooled_test_loader


def exp2_test_function(config, test_loader, model, aggregator, regressor, device):
    predicted_scores = []
    corresponding_name = []
    corresponding_mos = []
...
def test_model(config):
    ...
    if config.network_type == "regressor":
        aggregator = NormalRegressor(config.embed_dim)
...
def configuration_params():
    ...
    parser.add_argument('--test_result_dir', type=str,
                        default='/scratch/sanjotst/triplet_loss_runs/Run0034/')
    parser.add_argument('--test_dataset', type=list, default='pipal')
    parser.add_argument('--img_dir', type=str,
                        default='/home/sanjotst/llm_iqa/llm-iqa/datasets/PIPAL')
    parser.add_argument('--input_json_file', type=str,
                        default='/home/sanjotst/llm_iqa/llm-iqa/labels/pipal.json')
