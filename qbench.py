...
model = AutoModel.from_pretrained(
    'internlm/internlm-xcomposer-vl-7b', trust_remote_code=True).cuda().eval()
tokenizer = AutoTokenizer.from_pretrained(
    'internlm/internlm-xcomposer-vl-7b', trust_remote_code=True)
model.tokenizer = tokenizer

image_paths = [
    "/scratch/sanjotst/datasets/LIVE_IQA_new/",
    # "../datasets/1024x768/",
    # "../datasets/SPAQ/",
...
json_prefix = "/scratch/sanjotst/datasets/LIVE_IQA_new/"
jsons = [
    json_prefix + "live_iqa_new.json"
    # ,
    # json_prefix + "nnid.json",
...
for image_path, input_json in zip(image_paths, jsons):
    ...
    for i, llddata in enumerate(tqdm(iqa_data, desc=image_path)):
        message = "Rate the quality of the image <ImageHere> ."
        ...
        for image in images:
            ...
            lgood, lpoor = output_logits[0, 18682].item(
            ), output_logits[0, 5527].item()
