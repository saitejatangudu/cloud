...
from util_networks import get_internLM_model, BasicMultiheadAttentionAggregator, ComplexMultiheadAttentionAggregator, BasicMultiheadAttentionAggregator1, NormalRegressor1
...
def test_model(config):
    ...
    if config.network_type == "regressor":
        ...
        aggregator = BasicMultiheadAttentionAggregator(
            config.embed_dim, config.num_heads, regressor_bool=True)
    elif config.network_type == "c_attn":
    ...
    model, aggregator = load_model(
        model, aggregator, config.test_result_dir + 'Train/iter_11346.tar')
    ...
    with torch.no_grad():
        test_prediction, corresponding_mos, corresponding_name = exp2_test_function(
            config, pooled_test_loader, model, aggregator, device)
