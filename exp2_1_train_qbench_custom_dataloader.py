# Code for: Finetuning code for InternLM MLLM model.
#
# Adapted from: Sanjot- exp2_text_attention_encoding
# Created Date: 2 April 2024
# Last Modified Date: 9 April 2024
# Last Modified Author: Shika

# To do checklist for Shika:
# 1. Becnhmark QBench zero-shot performance with this code as a sanity check.
...
# From our files
from util_dataload import CustomDataset1, get_livefb_train_data, CustomDataset
from util_networks import AttentionAggregator, get_internLM_model, NormalRegressor1, BasicMultiheadAttentionAggregator, ComplexMultiheadAttentionAggregator
from exp2_test import exp2_test_function
...
"""
THIS FUNCTION IS REMOVED FROM CLASS BECAUSE:
- Calling this function on test file also, so want to do that without instantiating the class.
- Also, IMP_NOTE TO SHIKA: GO THROUGH THIS FUNCTION AFTER READING PAPER TO UNDERSTAND THE CODE!!! Tf is even happening here for now.
"""
...
class DNNIter(nn.Module):
    def __init__(self, config):
        ...
        if self.config.network_type == 'regressor':
            self.aggregator = NormalRegressor1(
                embed_dim=self.config.embed_dim).to(self.device)
        elif self.config.network_type == 'b_attn':
            # self.aggregator = AttentionAggregator(
            #     embed_dim=self.config.embed_dim, num_heads=self.config.num_mha_heads).to(self.device)
            self.aggregator = BasicMultiheadAttentionAggregator(
                self.config.embed_dim, self.config.num_mha_heads).to(self.device)
        elif self.config.network_type == 'c_attn':
            self.aggregator = ComplexMultiheadAttentionAggregator(
                embed_dim=self.config.embed_dim, num_heads=self.config.num_heads).to(self.device)
        ...
        # Identify the trainable parameters from self.model and self.aggregrator for optimization
        # trainable_parameters = [param for model in [self.model, self.aggregator] for name, param in model.named_parameters() if param.requires_grad]

        # if qformer trainable, get those parameters and self.aggregator parameters. Else, only self.aggregator parameters.
        # trainable_parameters = []
        # models = [self.aggregator, self.model]
        # for model in models:
        #     for name, param in model.named_parameters():
        #         if model == self.aggregator or any([param_name in name for param_name in self.config.optimizer_params]):
        #             if param.requires_grad_(True):
    ...
    def init_dataloaders(self):
        ...
        processed_train_data = CustomDataset1(
    ...
    def save_model(self, model, aggregator, optimizer, best=False):
        ...
        # save according to iteration number
        else:
        ...
        full_dict = {'model': save_model, 'optimizer': save_opt,
    ...
    def train_model(self):
        ...
        for iteration in tqdm(range(start_iteration, total_iterations + 1)):
            ...
            img_input = sampled_batch['img'].to(self.device)
            ...
            if self.config.logit_processing_type == 'init':
                hidden_states = get_init_logits(self.model, img_input)
            elif self.config.logit_processing_type == 'sentence':
                hidden_states = get_sentence_logits(
                    self.model, img_input, self.gen_config)

            # which type of network to use
            if self.config.network_type == 'regressor':
                scores = self.aggregator(hidden_states)
            elif self.config.network_type == 'b_attn':
                scores = self.aggregator(hidden_states)
            ...
            loss = self.get_loss_fn(scores, mos_target)
            ...
            print(
                f'Iteration {iteration} done with per closs {per_sample_loss:0.4f}.')
            ...
            del sampled_batch, img_input, mos_target, hidden_states, scores
    ...
    def test_during_train(self):
        with torch.no_grad():
            ...
            test_prediction, corresponding_mos, corresponding_name = exp2_test_function(
                self.config, self.pooled_test_loader, self.model, self.aggregator, self.device)
...
def configuration_params():
    ...
    parser.add_argument('--data_transform', type=str,
                        default='resize_interpolate')

    # network args
    parser.add_argument('--network_type', type=str,
                        default='b_attn')  # or attn
    parser.add_argument('--embed_dim', type=int, default=4096)
    parser.add_argument('--num_mha_heads', type=int, default=8)
    parser.add_argument('--internlm_trainable_params', type=list,
                        default=['Qformer'])
    parser.add_argument('--optimizer_params', type=list, default=['Qformer'])
    parser.add_argument('--logit_processing_type', type=str,
                        default='init')  # or init
    ...
    parser.add_argument('--resume_model_path', type=str,
                        default='/home/sanjotst/llm_iqa/llm-iqa/code/baselines/exp2_results_folder/Run0040/Train/iter_6614.tar')
    parser.add_argument('--regressor type', type=str,
                        default='We use customdataset1 in this experiment, it resizes images to 256 then center crops 224, the change in customdataset1 is             transforms.Resize(256), interpolation=InterpolationMode.BICUBIC) instead of transforms.Resize(256), we are having interpolation while resizing')
