...
class DNNIter(nn.Module):
    def __init__(self, config):
        ...
        if self.config.dataset == 'LIVE_FB_DIST2LEVELS5':
            self.img_dir = '/scratch/sanjotst/LIVE_FB/'
            self.num_distortions = 2
            self.num_levels = 5
            self.annotation_directory = self.config.annotation_directory_live_fb_2_dist_5_level_full
        ...
        if self.config.dataset == 'LIVE_FB_DIST4_LEVELS4':
            ...
            self.num_levels = 4
        if self.config.dataset == 'LIVE_FB_DIST1_LEVELS5':
            self.img_dir = '/scratch/sanjotst/LIVE_FB/'
            self.num_distortions = 1
            self.num_levels = 5
            self.annotation_directory = self.config.annotation_directory_live_fb_2_dist_5_level_full
            self.synthetic_img_dir = '/scratch/sanjotst/datasets/live_fb_synthetic/LIVE_FB_synthetic_full_v2'
        ...
        if self.config.model_description == 'llava':
            self.mllm_trainable_params = self.config.llava_trainable_params
        if self.config.model_description == 'mplug_owl':
            self.mllm_trainable_params = self.config.mplug_owl_trainable_params
        ...
        elif self.config.model_description == 'internlm_vl2_quantised':
            self.model = get_internLM_quantised_model(self.config)
        elif self.config.model_description == 'llava':
            self.model, self.tokenizer, self.image_processor = get_llava_model(
                self.config)
        elif self.config.model_description == 'mplug_owl':
            self.model, self.tokenizer, self.image_processor = get_mplug_owl_model(
                self.config)

        # set trainable params grad true
        ...
        if self.config.model_description != 'llav':
        ...
        if self.config.model_description == "llava":
            self.config.optimizer_params = self.config.llava_trainable_params
        if self.config.model_description == "mplug_owl":
            self.config.optimizer_params = self.config.mplug_owl_trainable_params
        if self.config.model_description == "internlm_vl":
            self.config.optimizer_params = self.config.internlm_trainable_params
        print(self.config.optimizer_params)
        ...
        for model in models:
            for name, param in model.named_parameters():
                # print(name)
                # print(
                #     [param_name in name for param_name in self.config.optimizer_params])
                # print("=======================================================")
                # print(
                #     any([param_name in name for param_name in self.config.optimizer_params]))
                if model == self.aggregator or model == self.regressor:
                    optimizable_parameters.append((name, param))
                    # print(" 1st test condition")
                elif any([param_name in name for param_name in self.config.optimizer_params]):
                    # print("outside 2nd condition")
                    if param.requires_grad:  # Use 'param.requires_grad' for checking
                        # print("2nd test condition")
                        optimizable_parameters.append((name, param))
                        self.mllm_optimizable_params.append((name, param))
                        # print(
                        #     f" the name of the parameter is : {name} the parameter dtype is  : {param.dtype}")
                else:
        ...
        # Construct the result directory name
        result_dir_name = f"{config.model_description}_{self.config.loss_function}_{self.config.dataset}_Run{run_number:04}"
        self.curr_result_dir = os.path.join(
            config.results_dir, result_dir_name)
        # Create the result directory if it does not exist
        if not os.path.exists(self.curr_result_dir):
            os.makedirs(self.curr_result_dir)
        # Update the configuration with the new result directory
    ...
    def convert_models_to_fp32(self, model):
        # add check for mplug owl also
        if self.config.model_description == 'llav':
            for p in model.parameters():
                p.data = p.data.half()
                # check if any value is becoming nan
                if torch.isnan(p.data).any():
                    raise ValueError("NaN detected in model parameter")
        else:
            for p in model.parameters():
                p.data = p.data.float()
        return
    ...
    def init_dataloaders(self):
        ...
        if self.config.dataset == 'LIVE_FB':
            processed_train_data = CustomDataset(
                df_data=train_data, img_dir=self.config.img_dir, model=self.config.model_description, data_transform=self.config.dataset_transform)
        elif self.config.dataset == 'LIVE_FB_DIST2LEVELS5':
            ...
            processed_train_data = CustomTrainDatasetAnnotatedLIVEFB(
                df_data=train_data, annotation_matrix_dir=self.annotation_directory,  synthetic_img_dir=self.synthetic_img_dir, model=self.config.model_description)
        elif self.config.dataset == 'LIVE_FB_DIST1_LEVELS5':
            processed_train_data = CustomTrainDatasetAnnotatedLIVEFB(
                df_data=train_data, annotation_matrix_dir=self.annotation_directory,  synthetic_img_dir=self.synthetic_img_dir, model=self.config.model_description)

        processed_test_data = CustomDataset(
            df_data=test_data, img_dir=self.config.img_dir, model=self.config.model_description, data_transform=self.config.dataset_transform)
    ...
    def train_model(self):
        ...
        for iteration in tqdm(range(start_iteration,  total_iterations + 1)):
            ...
            if self.config.test_at_first_iteration == 'true':
                if iteration == 1:
                    self.test_dict['test_srocc']['iter_no'].append(
                        self.current_iteration)
                    self.test_during_train()
            ...
            dist_img_input = sampled_batch['img'].to(
                self.device)  # bs, 17, c, h, w when loss isn't mse
            ...
            if self.config.loss_function != 'mse':
                ...
                dist_grouped = (dist_img_input.reshape(
                    b * d, c, h, w)).to("cuda")
            elif self.config.loss_function == 'mse':
                dist_grouped = dist_img_input
                # print(
                #     f" shape of batch of images in the case of mse loss is : {dist_grouped.shape}") bs * c * h * w
                # print(
                #     f" the datatype of this batch of images is : {dist_grouped.dtype}")  # torch.float16
                if self.config.model_description == 'llav':
                    dist_grouped = dist_grouped.half()
                    # print(
                    #     f" the datatype of this batch of images in case of llava is : {dist_grouped.dtype}")  # torch.float16
                    # print(dist_grouped.dtype)
                else:
                    pass
            ...
            if self.config.model_description == 'internlm_vl':
                if self.config.logit_processing_type == 'init':
                    hidden_states_dists = get_init_logits(
                        self.model, dist_grouped)
                ...
                if self.config.logit_processing_type == 'init':
                    hidden_states_dists = get_init_logits_internlm_v2(
                        self.model, dist_grouped)
                elif self.config.logit_processing_type == 'sentence':
                    hidden_states_dists = get_sentence_logits(
                        self.model, dist_grouped, self.gen_config)
            elif self.config.model_description == 'internlm_vl2_quantised':
                # dist_grouped = dist_grouped.to(self.config.default_device)
                # print(f"images device: {dist_grouped.device}")
                # print(
                #     f"model paramters device: {next(self.model.parameters()).device}")
                # # images are float32, convert them to fp16 for the quantised version
                # if dist_grouped.dtype != next(self.model.parameters()).dtype:
                #     dist_grouped = dist_grouped.to(
                #         next(self.model.parameters()).dtype)
                if self.config.logit_processing_type == 'init':
                    hidden_states_dists = get_init_logits_internlm_quantised(
                        self.model, dist_grouped, self.config.default_device)
                elif self.config.logit_processing_type == 'sentence':
                    hidden_states_dists = get_sentence_logits(
                        self.model, dist_grouped, self.gen_config)
            elif self.config.model_description == 'llava':
                if self.config.logit_processing_type == 'init':
                    hidden_states_dists = get_init_logits_llava(
                        self.model, dist_grouped, self.tokenizer, self.image_processor)
                elif self.config.logit_processing_type == 'sentence':
                    hidden_states_dists = get_sentence_logits(
                        self.model, dist_grouped, self.gen_config)
            elif self.config.model_description == 'mplug_owl':
                if self.config.logit_processing_type == 'init':
                    hidden_states_dists = get_init_logits_mplug_owl(
                        self.model, dist_grouped, self.tokenizer, self.image_processor)
                elif self.config.logit_processing_type == 'sentence':
                    hidden_states_dists = get_sentence_logits(
                        self.model, dist_grouped, self.gen_config)
            ...
            # print("printing hidden states which are sent into aggregator")
            # print(hidden_states_dists.shape)

            attention_output_dists = self.aggregator(
                hidden_states_dists)
            # print(attention_output_dists.dtype)

            if self.config.loss_function != 'mse':
                # Do feature level contrastive learning on the attention outputs of distorted versions of images
                attention_output_dists = attention_output_dists.reshape(
                    b, d, -1)
                ...
                # print(f"the shape of scores from regressor for mse is  : {scores.shape}")
            else:
                scores = self.regressor(attention_output_dists)
                # print(scores.shape)
                # print(mos_target.shape)

            if scores.shape != mos_target.shape:
                scores = scores.squeeze()
                mos_target = mos_target.squeeze()
                if self.config.model_description == "llav":
                    mos_target = mos_target.half()
                else:
                    pass
                # print(f" datatype of mos is : {mos_target.dtype}")
            print(scores)
            ...
            if self.config.loss_function == 'self_supervised_loss_1':
                ...
                # print(f" the shape of torch tensor sent into the ssl1 loss : {attention_output_dists.shape}")
                # print(f"number of distortions : {self.num_distortions}")
                # print(f"number of levels of distortions : {self.num_levels}")
                loss1 = loss_fn1(attention_output_dists,
                                 self.num_distortions, self.num_levels)
                loss_contrastive = loss1
                total_loss = loss_mse + self.config.alpha_scaling * loss_contrastive
                self.logger.add_scalar(
                    'LevelBlendedOrderingLoss', loss1, self.current_iteration)
                loss = total_loss
            ...
            if self.config.loss_function == 'self_supervised_loss_2':
                ...
                loss1 = loss_fn1(attention_output_dists,
                                 self.num_distortions, self.num_levels)
                loss2 = loss_fn2(
                    attention_output_dists, annotator_matrix, self.num_distortions, self.num_levels)
                loss_contrastive = loss1 + loss2
                total_loss = loss_mse + self.config.alpha_scaling * loss_contrastive
                self.logger.add_scalar(
                    'LevelBlendedOrderingLoss', loss1, self.current_iteration)
                self.logger.add_scalar(
                    'Opt1_DistBlendedOrderingLoss', loss2, self.current_iteration)
                loss = total_loss
            ...
            if self.config.loss_function != 'mse':
                ...
                del sampled_batch, dist_img_input, mos_target, hidden_states_dists, attention_output_dists, scores
    ...
    def test_during_train(self):
        with torch.no_grad():
            ...
            # Ensure that tokenizer and image_processor are passed if model requires them
            if self.config.model_description in ['llava', 'mplug_owl']:
                test_prediction, corresponding_mos, corresponding_name = get_predictions(
                    self.config, self.pooled_test_loader, self.model, self.aggregator, self.regressor, self.device, self.tokenizer, self.image_processor
                )
            else:
                test_prediction, corresponding_mos, corresponding_name = get_predictions(
                    self.config, self.pooled_test_loader, self.model, self.aggregator, self.regressor, self.device
...
def configuration_params():
    ...
    parser.add_argument('--dataset_transform', type=str,
                        default='normal_transform')

    # network args or model arguments
    parser.add_argument('--network_type', type=str, default='b_attn')
    parser.add_argument('--regressor_type', type=str, default='complex_r')
    ...
    parser.add_argument('--internlm_v2_trainable_params',
                        type=list, default=['Plora'])
    parser.add_argument('--internlm_quantised_trainable_params',
                        type=list, default=['Plora'])
    parser.add_argument('--llava_trainable_params',
                        type=list, default=['projector'])
    parser.add_argument('--mplug_owl_trainable_params',
                        type=list, default=['visual_abstractor'])
    parser.add_argument('--optimizer_params', type=list, default=['Plora'])
    ...
    parser.add_argument('--resume_model_path', type=str,
                        default='/scratch/sanjotst/experiment_results/mplug_owl_mse_LIVE_FB_Run0502/Train/iter_7564.tar')
...
def main():
    ...
    #     print("setting visible devices to 7")
